...
if (!empty($_GET['member_id'])) {
    $members_to_print = array(Dashboard_DB::get_member_by_id(intval($_GET['member_id'])));
} else {
    $members_to_print = Dashboard_DB::get_members();
}
...
<head>
    ...
    <style>
        ...
        .dashboard-logo { height: 50px; margin-bottom: 10px; }
...
<body>
    <div class="cards-wrapper">
        <?php foreach ($members_to_print as $member):
            if (!$member) continue;
            $pass = get_user_meta($member->wp_user_id, 'dashboard_temp_pass', true);
            if (empty($pass)) $pass = '********';
        ?>
        <div class="card">
            <div class="card-header">
                <?php if ($dashboard['dashboard_logo']): ?>
                    <img src="<?php echo esc_url($dashboard['dashboard_logo']); ?>" class="dashboard-logo">
                <?php endif; ?>
                <div style="font-size: 14px; font-weight: 700;"><?php echo esc_html($dashboard['dashboard_name']); ?></div>
            </div>
            <div class="card-body">
                ...
                <div class="note">
                    * يستخدم هذا الحساب للدخول لنظام Dashboard (العضو وولي الأمر).<br>
